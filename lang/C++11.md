...
enum class CharColor: unsigned char  
{
   red = 0x1,
   blue = 0x2, 
   green = 0x3,
...
{  
   ...
   std::cout << "is_same<unsigned char> " << std::is_same<unsigned char,        	  								decltype(CharColor::blue)>::value << std::endl;
      
   return 0;
}

>> is_same<unsigned char> false
```

Leider gibt die Type Traits Bibliothek ein 'false' bei der Überprüfung des Typs. Wieso? Null Ahnung. Ich hätte etwas anderes erwartet (getestet mit gcc v10 und C++11).
