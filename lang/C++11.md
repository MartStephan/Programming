...
* [Table of Contents](#table-of-contents)
  ...
  * [Deleted specifier](#deleted-specifier)
  * [Overriding specifier](#overriding-specifier)
...
Schlussendlich gibt es noch das Schlüsselwort final, mit dem angezeigt wird, dass die Funktion virtual ist und sie nicht überschrieben werden darf von abgeleiteten Klassen.
...
int& getRef() { static int si; return si; }

int i = 1; 	// i ist Lvalue; 1 ist Rvalue

int& lvalueRef = i; 	// lvalueRef ist Lvalue Referenz, i ist Lvalue

int& r1 = getFour();    // Compile-time error: cannot bind lvalue-reference-to-non-const to an rvalue (ever since C++84) (and rightly so -- the user could then modify something efemeral -- probably not what was meant).
const int& r2 = getFour();     // Allowed -- similar to having a const-lvalue, except further passing it might be more efficient. Lifetime of temporary is extended to match lifetime of reference: (GotW #88)[http://herbsutter.wordpress.com/2008/01/01/gotw-88-a-candidate-for-the-most-important-const/].
...
MyData&& rvalueRef = MyData();   // Rvalue Referenz -- itself an lvalue because it has a name. Extends lifetime (just like lvalue-reference-to-const).
```
...
Mit der *std::chrono* Bibliothek wird das Benutzen von Zeiten, Zeiträumen, Zeitpunkten und Datums- und Uhrzeiten deutlich vereinfacht. Die Bibliothek besteht aus folgenden drei (Haupt-)Komponenten: 
...
int main()
{
   // time point: Definiere Start- und Endzeitpunkt
   std::chrono::time_point<std::chrono::system_clock> start;
   std::chrono::time_point<std::chrono::system_clock> end;
   
   // Ermittle jetzt-Zeitpunkt mit Hilfe von 'clock'
   ...
   // (end - start) ergibt Zeitdauer - nutze hierfür 'clock'
...
[32] Ressourcen sparen mit der C++ - Funktion contexpr, Andreas Fertig, iX 12/2024
